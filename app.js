...
const client = new Discord.Client({disableEveryone: false, intents: [Discord.Intents.FLAGS.GUILDS, Discord.Intents.FLAGS.GUILD_MESSAGES, Discord.Intents.FLAGS.GUILD_MESSAGE_TYPING, 32768]});
...
client.on('message', async msg => {
    ...
    if(!msg.author.bot){ 
        ...
        if (msg.content === '!cagon' || msg.content === '!cagón') {
            if (msg.member.voice.channel) {
                const connection = await msg.member.voice.channel.join();
                await SoundsController.playCagon(connection,msg);
            } else {
                msg.reply('Debes estar en un canal de voz!');
            }
        }
        ...
        if (msg.content === '!hey') {
            if (msg.member.voice.channel) {
                const connection = await msg.member.voice.channel.join();
                await SoundsController.playHey(connection,msg);
            } else {
                msg.reply('Debes estar en un canal de voz!');
            }
        }
        ...
        if (msg.content === '!culos') {
            if (msg.member.voice.channel) {
                const connection = await msg.member.voice.channel.join();
                await SoundsController.playCulos(connection,msg);
            } else {
                msg.reply('Debes estar en un canal de voz!');
            }
        }
        ...
        if (msg.content === '!porQueSiguesConEl') {
            if (msg.member.voice.channel) {
                const connection = await msg.member.voice.channel.join();
                await SoundsController.playPorQueSigues(connection,msg);
            } else {
                msg.reply('Debes estar en un canal de voz!');
            }
        }
